...
// Passo 1: Garantir que a Keystore existe via caminhos relativos
...
// Passo 2: Limpar twa-manifest.json antigo se houver para reconfiguração limpa
const manifestPath = path.join(cwd, 'twa-manifest.json');
if (fs.existsSync(manifestPath)) {
  console.log('[CLEAN] Removendo twa-manifest.json antigo...');
...
// Inicia o processo Bubblewrap 'init'
...
init.stdout.on('data', (data) => {
  ...
  // Normaliza o output para busca case-insensitive robusta
...
init.on('close', (code) => {
  ...
  // Redireciona entrada padrão para o build
  process.stdin.unpipe(init.stdin);
  process.stdin.pipe(build.stdin, { end: false });
  ...
  build.stdout.on('data', (data) => {
    ...
    // Responder senhas de assinatura com locks individuais
  ...
  build.on('close', (buildCode) => {
    ...
    const aabSource = allFiles.find(f => f.endsWith('-release-signed.aab'));
